function [point,distance] = search(patient_brain,EEG_list,res_x,res_y,res_z,searchend)
...
distance = zeros(19,1);
% upper_buffer = zeros(size(patient_brain,1),size(patient_brain,2),30);
% patient_brain = cat(3,upper_buffer,patient_brain);
patient_brain = int8(patient_brain);
for iv = 1:searchend
    fprintf('\nSearching for closest point to EEG point %d\n',iv)
        ...
        for iii = z-35:1:z
            for ii = y-30:1:y+30
                for i = x-30:1:x+30
                    ...
                    if i>size(patient_brain,1)
                        xi = size(patient_brain,1);
                    elseif i<1
                        xi = 1;
                    else
                        xi = i;
                    ...
                    if ii>size(patient_brain,2)
                        yii = size(patient_brain,2);
                    elseif ii<1
                        yii = 1;
                    else
                        yii = ii;
                    ...
                    if iii>size(patient_brain,3)
                        ziii = size(patient_brain,3);
                    elseif iii<1
                        ziii = 1;
                    else
                        ziii = iii;
                    ...
                    value_list(it) = patient_brain(xi,yii,ziii);

                    if patient_brain(xi,yii,ziii) > 150
                        max_Coordinates(it,:) = [coordinate_list(it,1),coordinate_list(it,2),coordinate_list(it,3)];
                        max_Value(it) = patient_brain(coordinate_list(it,1),coordinate_list(it,2),coordinate_list(it,3));
                        distances(it) = sqrt(...
